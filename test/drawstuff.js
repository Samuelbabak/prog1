...
LIGHT_COORDINATES = (-3, 1, -0.5)

EYE_LOCATION = Vector((0,0,0), (0.5,0.5,-0.5))
VIEW_UP_VECTOR = Vector(0, 1, 0)
LOOK_AT_VECTOR = (0, 0, 1)

WINDOW_DISTANCE = 0.5
ASPECT_RATIO = 1
WINDOW_CENTER = (0.5, 0.5, 0)
VERTICAL_ANGLE = 0.45
HORIZONTAL_ANLE = 0.45

SCREEN_DIMENTIONS_X = WINDOW_DISTANCE

//put random points in the triangles from the class github
function drawTriangles(context) {
    // var inputTriangles = getInputTriangles();
    t = require('./triangles2.json');
    let inputTriangles = t
        ...
        for (var f=0; f<n; f++) {
        	...
        	for(var t=0; t<tn; t++){
            	...
            	for (var width=0; width<w; width++) {
                    for(let len=0; len<h; len++) {
                        var point; // on canvas plane
                        let vector_eye2Pixel = Vector()
                        var triangleTest = 0;
